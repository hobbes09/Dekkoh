...
public class AnswerFragmentListAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView,
        ...
        if (convertView == null) {
            /*
             * if (position % 2 != 0) { if(presentAnswer.getImage()==null ||
             * presentAnswer.getImage().compareTo("")==0){ itemView = (RelativeLayout)
             * mLayoutInflater.inflate( R.layout.answers_fragment_listview_cell_left_withoutimage,
             * parent, false); userName = (TextView) itemView
             ...
             * }else{ Log.e("Answer Image",presentAnswer.getImage()); itemView = (RelativeLayout)
             * mLayoutInflater.inflate( R.layout.answers_fragment_listview_cell_left, parent,
             * false); userName = (TextView) itemView
             * .findViewById(R.id.userNameInAnswerCellUserAnswerFragmentLeft); userLocation =
             * (TextView) itemView
             * .findViewById(R.id.answerCell_question_fragment_userLocationLeft); answerOfUser =
             * (TextView) itemView .findViewById(R.id.answerInAnswerCellUserAnswerFragmentLeft);
             * userProfilePic = (ImageView) itemView
             * .findViewById(R.id.profileImageInAnswerCellUserAnswerFragmentLeft); answerImage =
             * (ImageView) itemView .findViewById(R.id.answers_fargemnt_imageAnswerImageViewLeft);
             * if(convertView==null){ Picasso.with(mContext) .load(presentAnswer.getImage())
             * .transform(transformation) .into(answerImage); } } } else {
             */
            if (presentAnswer.getImage() == null || presentAnswer.getImage().compareTo("") == 0
                    || !presentAnswer.getImage().contains("http")) {
