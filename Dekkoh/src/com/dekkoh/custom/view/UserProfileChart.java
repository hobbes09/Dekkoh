...
public class UserProfileChart extends ViewGroup {
     ...
     * layout engine to construct a {@link UserProfileChart} from a set of XML attributes.
     * 
     * @param context
     * @param attrs An attribute set which can contain attributes from
     *            {@link com.UserProfileChart.android.customviews.R.styleable.UserProfileChart} as
     *            well as attributes inherited from {@link android.view.View} .
     */
    ...
    /**
     * Set the strength of the highlighting that is applied to each pie segment. This number is a
     * floating point number that is multiplied by the base color of each segment to get the
     * highlight color. A value of exactly one produces no highlight at all. Values greater than one
     * produce highlights that are lighter than the base color, while values less than one produce
    ...
    /**
     * Set the currently selected item. Calling this function will set the current selection and
     * rotate the pie to bring it into view.
     * 
    ...
    /**
     * Set the current item by index. Optionally, scroll the current item into view. This version is
     * for internal use--the scrollIntoView option is always true for external callers.
     * 
     * @param currentItem The index of the current item.
     * @param scrollIntoView True if the pie should rotate until the current item is centered. False
     *            otherwise. If this parameter is false, the pie rotation will not change.
     */
    ...
    /**
     * Add a new data item to this view. Adding an item adds a slice to the pie whose size is
     * proportional to the item's value. As new items are added, the size of each existing slice is
     * recalculated so that the proportions remain correct.
     * 
    ...
    private void onDataChanged() {
        ...
        for (Item it : mData) {
            ...
            it.mShader = new SweepGradient(mPieBounds.width() / 2.0f,
                    mPieBounds.height() / 2.0f, new int[] {
                            it.mColor,
                            it.mColor, it.mColor, it.mColor,
                    }, new float[] {
    ...
    /**
     * Kicks off an animation that will result in the pointer being centered in the pie slice of the
