...
private DekkohApplication dekkohApplication;
	...
	private String questionAskerUserName,questionAskerUserProfilePicUrl,questionAskerUserQuestion,questionAskerUserLocation,questionId;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		questionId = incoming.getString("questionId");
				...
				//Initialization
				userProfilePic = (ImageView)root.findViewById(R.id.post_answer_fragment_answering_userProfileImage);
				userAnswerImage = (ImageView)root.findViewById(R.id.post_answer_fragment_answering_user_layout);
				userName=(TextView)root.findViewById(R.id.post_answer_fragment_answering_userName);
				userLocation = (TextView)root.findViewById(R.id.post_answer_fragment_answering_userLocation);
				userAnswer = (EditText)root.findViewById(R.id.post_answer_fragment_edittext_for_user_answer);
				postAnswer = (Button)root.findViewById(R.id.post_answer_fragment_postbutton);
				questionAskerProfilePic = (ImageView)root.findViewById(R.id.post_answer_fragment_questioned_user_ProfileImage);
				questionAskerName = (TextView)root.findViewById(R.id.post_answer_fragment_questioned_user_Name);
				questionAskerLocation = (TextView)root.findViewById(R.id.post_answer_fragment_questioned_user_Location);
				...
				postAnswer.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
					  ...
					  if(userAnswer.getText().toString().compareTo("")!=0){
						  ...
						  new PostAnswer(PostAnswerFragment.this,userAnswer.getText().toString(),questionId,userLocation.getText().toString(),String.valueOf(dekkohApplication.getLocationLatitude()),String.valueOf(dekkohApplication.getLocationLongitude()),dekkohUser.getProfilePic(),"send answer pic url here").execute();
					  }
					}
				});
...
class PostAnswer extends AsyncTask<Void,Void,Void>{
	...
	public PostAnswer(PostAnswerFragment parent,String answer,String questionId,String location,String lati,String longi,String userPictureUrl,String answerPicUrl){
	...
	protected Void doInBackground(Void... params) {
		...
		if(hasActiveInternetConnection(parent.getActivity().getApplicationContext())){
			no_net=false;
			try {
				APIProcessor.postAnswer(parent.getActivity(), answer, questionId, answerPicUrl, location, String.valueOf(System.currentTimeMillis()/1000L));
			} catch (Exception e) {
